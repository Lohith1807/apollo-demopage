...
import { getSubjects, getFaculty } from '../../services/api';
import {
    Book, FileText, Video, Clipboard,
    MoreVertical, Download, ExternalLink,
    MessageSquare, Calendar, ChevronRight,
    Clock, Plus, Bookmark, Filter, ArrowLeft, Phone, User as UserIcon
} from 'lucide-react';

const ClassroomCube = ({ course, onClick, isStaff: _isStaff }) => (
    <button
        onClick={onClick}
        ...
        <div className="p-6 flex flex-col h-full w-full">
            ...
            <div className="mt-4 space-y-2">
                <div className="flex items-center gap-2 text-[10px] font-black text-slate-600 uppercase tracking-tight">
                    <div className="w-4 h-4 rounded-full bg-apollo-red/10 flex items-center justify-center">
                        <UserIcon size={10} className="text-apollo-red" />
                    </div>
                    {course.teacher}
                </div>
...
export default function Classroom() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [subRes, facRes] = await Promise.all([getSubjects(), getFaculty()]);
    ...
    const courses = rawSubjects.map((name, i) => {
        const subjectName = typeof name === 'string' ? name : (name.name || 'Subject');
        const teacherInfo = findTeacher(subjectName);
        return {
            id: i,
            name: subjectName,
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                            ...
                            <div className="p-6 space-y-4">
                                <div className="flex gap-4 p-4 bg-slate-50 rounded-xl">
                                    <div className="w-10 h-10 rounded-full bg-apollo-red flex items-center justify-center text-white font-black text-xs">FAC</div>
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Classroom</h1>
                    <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">Classroom Hub • Welcome, {user?.name}</p>
                </div>
                <div className="flex gap-2">
            ...
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-8 text-white relative overflow-hidden shadow-2xl">
                <div className="relative z-10 max-w-xl">
                    <h2 className="text-2xl font-black uppercase tracking-tight">Main Classroom Hub</h2>
